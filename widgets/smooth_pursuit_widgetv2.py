...
class SmoothPursuitWidget(QWidget):
    ...
    def init_ui(self):
        """Initialize the UI with top and bottom layout."""
        layout = QVBoxLayout(self)  # Use vertical layout for top and bottom
        ...
        # Top part for countdown
        self.countdown_widget = QWidget()
        countdown_layout = QVBoxLayout(self.countdown_widget)
        
        # Create a label to display the countdown
        self.countdown_label = QLabel(str(self.countdown_seconds), self)
        self.countdown_label.setAlignment(Qt.AlignCenter)
        self.countdown_label.setStyleSheet(f"color: rgb({self.text_color.red()}, {self.text_color.green()}, {self.text_color.blue()})")
        countdown_layout.addWidget(self.countdown_label)
        
        # Add countdown widget to the top part of the layout
        layout.addWidget(self.countdown_widget, 1)  # Top half
        
        # Bottom part: Initially, only the countdown label will be visible
        self.display_widget.hide()  # Initially hide the display widget (for moving dots)
    ...
    def update_countdown(self):
        ...
        if self.countdown_value < 0:
            ...
            # Remove countdown and switch to full screen for animation
    ...
    def log_point(self, point):
        ...
        with open(self.log_file, 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([timestamp_ms, self.current_row, self.current_point, point.x(), point.y(), screen.width(), screen.height()])
